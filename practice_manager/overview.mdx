...
Practice Manager handles billing, not clinical records, so the EHR you use today stays where it is. Practice Manager connects to 50+ EHRs and syncs appointments, encounters, charges, and patient files on a schedule.

Run Air, our EHR, and there is nothing to connect: a signed note becomes a billable charge in the same system. If you are migrating onto Air, billing keeps running on Practice Manager the whole time, so collections never pause for the cutover.

A few capabilities only apply if you run Air, mainly scheduling and check-in. Those rows are marked.
