...
description: "Review claim status, submissions, remittances, payments, and activity in one workspace."
toc: "true"
---
...
- **Remittances** — remittance data with its source, deposit-verification status, and **Member ID** when available. You can download the raw **835 (ERA)** file or a PDF.
...
| **Event** | **What it means** |
| :-- | :-- |
| **Drafted a claim submission** | A submission draft was generated for the claim. |
| **Queued a claim for submission** | A claim, corrected claim, resubmission, or void was placed in the submission queue. |
| **Submitted a claim** | A claim, corrected claim, resubmission, or void was sent to its destination. |
| **Reviewed the claim** | Validation either passed or found issues that require review. |
| **Failed to submit a claim** | The submission or resubmission failed. Expand the event to review the **Failure reason**. |
...
When several field updates occur within the same minute, the feed shows one timestamp on the parent event. If the updates span different minutes, each field displays its own time.
