...
description: "Learn how to organize, review, and submit claims from the Insights Claims page."
---
...
The filter bar at the top of the table lets you narrow your view by any combination of attributes: claim status, insurance provider, claim amount, age, assigned team member, provider, date range, Submission ID, Insurance Billing Type (Workers’ Comp, Auto, or Commercial), and CARC/RARC denial reason codes.
...
### Display hidden claims

The **Show Pre-Athelas Claims** option under **Display** is off by default. If this option hides every claim that matches your filters, the empty state shows how many claims are hidden by display options. Select **Show options** to open the Display panel and change the setting.

### Use work queues

Use the **Work Queue** column or filter to organize claims by their current processing step. Work queue labels include:
...
### Review patient attachments in Claim Context

Open **Patient Attachments** in Claim Context to review documents associated with the claim. Encounter medical records appear under **Medical File**, while submitted claim documents appear under **Claim**. Select a document to preview it without leaving the claim.

### **Using Athelas Assistant**

Athelas Assistant is your AI-powered copilot inside every claim. Open it by clicking the **Athelas AI** button in the top navigation bar while viewing a claim. It understands the context of the claim you’re working on and can take action on your behalf.
...
In the **Procedures** section, use the add control to restore a procedure that a billing rule removed. The picker only shows eligible procedures that were already on the claim; you cannot create an unrelated procedure from this control.
...
For claims with more than one payer, a secondary destination remains unavailable while the primary submission is queued or otherwise unavailable. The disabled option explains why you cannot submit yet.
...
<Accordion title="How do I quickly copy a Claim ID?">
...
<Accordion title="How do I filter claims by denial reason codes (CARC/RARC)?">
...
<Accordion title="Why are no claims showing even though my filters should match?">
Open **Display** and check **Show Pre-Athelas Claims**. When display options are hiding matching claims, select **Show options** from the empty state to open the setting directly.
</Accordion>

<Accordion title="Can I add any procedure while editing a submission?">
No. The **Procedures** add control only restores eligible procedures that were already on the claim and were removed by billing rules.
</Accordion>