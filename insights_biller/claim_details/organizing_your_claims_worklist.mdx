...
![Claims grouped by work queue and ordered by balance](/images/insights_biller/claim_details/organizing_your_claims_worklist/organizing_your_claims_worklist_1.png)

You can group by **work queue**, **reason**, **site**, and other fields. Within a grouped list you can:
...
![A single work queue grouped by reason for an individual biller](/images/insights_biller/claim_details/organizing_your_claims_worklist/organizing_your_claims_worklist_2.png)

<Tip>
✨**Smart Tip:** A common team setup — **managers** group **all claims by work queue** and order by balance to see the full revenue funnel; **individual billers** filter to one work queue, **group by reason**, order by count or balance, and save that as a view so they always work the highest-leverage bucket first.
</Tip>
...
<Accordion title="What's the difference between filtering, grouping, and views?">
They stack together:

- **Filtering** narrows *which* claims appear (e.g., one provider, this week).
- **Grouping** organizes the claims that remain into collapsible buckets (e.g., by reason).
- **Views** save a combination of filters, grouping, and ordering under a name so you can return to it instantly.

The most durable setups combine all three and save them as a view.
</Accordion>

<Accordion title="Will my saved views be visible to my teammates?">
No. Views are personal and stored per user. They persist across sessions, devices, and sites, but each teammate builds and sees their own. You can duplicate one of your own views as a starting point — right-click it and select **Duplicate**.
</Accordion>

<Accordion title="Why can I only select 50 claims at a time?">
It's a performance safeguard that keeps the page responsive on all machines. To act on a larger group, scroll down to load more rows, then select again — each selection grabs everything currently loaded. Scroll to the bottom of a group first, then select all to capture the whole bucket.
</Accordion>

<Accordion title="Why should I group by reason?">
Grouping by reason merges submission errors, rejections, and denials into one prioritized worklist, with each reason in its own bucket. It lets you find repeatable fixes fast — work all claims with the same reason together, then act on the whole bucket at once with **More Actions**.
</Accordion>
