---
title: "Deferring (Snoozing) a Claim"
...
Sometimes a claim has work to be done — but not right now. Maybe you're waiting on a callback from a payer, an appeal in litigation, or a coding update from a provider. **Deferring** lets you snooze a claim so it drops out of your worklist until it's actionable again, without losing track of it.
...
<Warning>
Deferring **does not change the work queue** — it only "snoozes" the claim in its current queue. The date, reason, and duration of every deferral are recorded in the [Activity Feed](/insights_biller/claim_details/working_a_claim).
</Warning>
...
## Which Claims Can Be Deferred

You can only defer **workable claims** — the same set of claims that appear in the **Workable Claims** view. If a claim isn't workable (for example, it's already finalized or waiting on a payer decision), the **Defer** option won't be available.

![The Actions menu on a claim that cannot be deferred](/images/insights_biller/claim_details/deferring_a_claim/deferring_a_claim_4.png)
...
- You **can't** defer a batch if even one claim in it is ineligible. Remove the ineligible claim and try again.
...
No. Deferring only snoozes the claim in its current work queue and hides it from your workable views. When the deferral expires — if the work queue hasn't advanced on its own — the claim reappears exactly where it was.
</Accordion>
