...
<Update label="Jan 31, 2026" tags={["Insights"]}>
  ...
  ### [Beta user only] Charge Master released to pilot sites
  ...
  **Eligibility & Benefits:**
  - Fixed insurance deductible and remaining benefits calculations
  - Corrected appointment eligibility counts displayed to staff
  - Fixed eligibility timestamp to show in user's local timezone

  **Collections & Reporting:**
  - Fixed outstanding balance calculations in end-of-day emails
  - Corrected patient exclusion logic in daily collections reports

  **Claims & Encounter Details:**
  - Fixed diagnosis code saving errors
  - Resolved date range filtering issues across multiple pages
  - Fixed various posting tool and encounter details errors
</Update>

<Update label="Dec 19, 2025" tags={["Insights"]}>

  ### [Beta user only] Claims Page launches

  On December 19, 2025, we launched the new Claims Page to select beta sites. The complete professional claims flow includes submission preview (view CMS and UB claims as PDFs before submitting), rules visibility showing which billing rules applied and what changed, activity feed for notes and claim history, side-by-side panels to view claim, submission preview, PDF, and rules simultaneously, and Athelas AI copilot for intelligent assistance.

  The new claims details endpoint runs **more than 3x faster** than the previous version. Available to beta sites with broader rollout planned for Q1 2026.

  ### [Beta user only] AI Report Builder beta release

  On December 15, 2025, we launched the AI Report Builder to 4 external beta sites. Teams can ask questions in plain English and get instant reports backed by their data, with natural language queries, automatic SQL generation with security post-processing, patient-level security enforcement, saved reports library, CSV downloads, and quality checking system.

  ### [Beta user only] Remittances Dashboard released

  The Remittances Overview Dashboard provides comprehensive cash flow visibility with waterfall charts visualizing deposits flowing through your system, dynamic grouping by deposit source, matching status, facility, or provider, score cards showing key metrics, drill-down navigation to click any chart element and see underlying transactions, and filtering across all views.
  ...
  - Multiple other sites seeing 10-30x improvements
  ...
  **Claims & EOB:**
  - Fixed currency conversion errors in EOB remittance posting
  - Improved claims history query performance (significantly faster)
  - Fixed claim form generation speed (2x faster)
  - Fixed EOB posting for large files (now handles 2,800+ remittances)

  **User Interface:**
  - Fixed denials page scrolling and navigation issues
  - Fixed user logout errors when working with deposits
  - Corrected currency display in downloaded reports (cents to dollars)
  - Added security masking for bank account numbers
  - Fixed encounter details error code display
</Update>
...
<Update label="Nov 30, 2025" tags={["Insights"]}>
  ...
  **Payments & Refunds:**
  - Fixed credit card refund processing errors
  - Resolved subscription balance calculation issues
  - Fixed patient responsibility cancellation errors

  **Patient Statements:**
  - Fixed paper statement delivery failures at multiple sites
  - Resolved statement mailing service issues

  **Patient Workflows:**
  - Fixed verification code timing issues that blocked payments
  - Resolved workflow errors affecting patient check-in
</Update>
...
<Update label="Nov 30, 2025" tags={["Air"]}>
  ...
  The AI Scribe now works seamlessly with the redesigned EHR and dynamic template system. Notes generated by Scribe automatically populate the correct template sections, saving providers time on documentation. Existing sites can now migrate to the new EHR design with full Scribe functionality.
</Update>
...
<Update label="Oct 25, 2025" tags={["Air"]}>
  ...
  We've dramatically improved the data migration process for new sites. Migrations from WebPT, Raintree, and Empower now complete in 15-30 minutes instead of 6-8 hours. We've successfully migrated over 20,000 chart notes across 14 sites with minimal manual intervention.
  ...
  **Chart Notes:**
  - Fixed auto-save reliability issues
  - Resolved template editing and conversion errors
  - Improved dynamic template rendering

  **Permissions:**
  - Billing managers and admins can now edit and submit notes
  - Enhanced role-based access controls

  **Data Management:**
  - Added delete/archive capabilities for cases, insurances, and prior authorizations
  - Fixed appointment editing issues
</Update>
...
<Update label="Oct 31, 2025" tags={["Insights"]}>
  ...
  We've migrated sites to Stripe's Separate Charges & Transfers (SC&T) infrastructure, providing better payment handling and financial controls:
  - Customer mapping for sites with Stripe accounts per facility or provider
  ...
  These improvements make patient check-in and paperwork submission smoother and more reliable across all sites.
  ...
  **Payments:**
  - Fixed issues with credits being incorrectly applied to subscriptions
  - Resolved payment processing timeouts
  - Fixed gift card payment errors

  **Patient Statements:**
  - Fixed patients not receiving statements at multiple sites
  - Corrected mailing address display issues
  - Fixed statement scheduling errors
  - Corrected batch history balance calculations
</Update>
...
<Update label="Sep 17, 2025" tags={["Air"]}>
  ...
  **Forms & Intake:**
  - Added Spanish language intake documents
  - Fixed duplicate insurance entry issues

  **Faxing:**
  - Enabled sending multiple files in a single fax
  - Improved Plan of Care fax tracking and delivery rates
  - Implemented zero-downtime fax transition for new sites
</Update>
...
<Update label="Sep 30, 2025" tags={["Insights"]}>
  ...
  Available at select sites. Contact your account manager to join the beta program.
  ...
  Currently available to pilot sites.
  ...
  **Patient Statements:**
  - Fixed statements not being sent for guarantors with dependents
  - Resolved duplicate statements appearing in downloaded ZIP files
  - Fixed electronic statement delivery failures

  **Payments & Billing:**
  - Fixed incorrect copay calculations for self-pay appointments
  - Corrected Stripe transaction fee errors
  - Improved appointment reminder delivery reliability
</Update>
