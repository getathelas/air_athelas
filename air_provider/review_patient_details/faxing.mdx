...
**Note:** For the quick, task-focused version of this workflow, see [Send & Receive Faxes](/air_front_desk/manage_your_tasks/faxes).
...
**Dynamic variables.** Beyond text boxes, templates automatically populate context from the fax — `{{patient.name}}`, `{{provider.npi}}`, `{{facility.fax}}`, `{{document.page_count}}`, and more. Staff don't have to fill these in; Air resolves them at send time.
