...
<Accordion title="What's the benefit of enabling Text Snippets for Scribe vs. not enabling them?">
  When enabled for Scribe, your snippets automatically shape the AI-generated note — standardizing phrasing, adding clinical context, and controlling formatting — so you spend less time editing after each visit. If a snippet is **not** enabled for Scribe (the **Use for EHR Scribe** toggle is off), it has no effect on AI note generation.
</Accordion>

<Accordion title="What's the difference between Replacement, Context, and Format macros?">
  **Replacement** swaps a trigger phrase for exact expanded text directly in the transcription before the note is written. **Context** gives the AI extra clinical guidance to influence how the note reads, without changing the transcription. **Format** controls structure and conciseness (e.g., bullets, brief vs. detailed). Only Replacement modifies the transcription text.
</Accordion>

<Accordion title="Why isn't my snippet showing up in Scribe?">
  Check that **Use for EHR Scribe** is enabled, a **Macro Type** is set, and the snippet's **Appointment Types** include the visit (or leave them empty to apply globally). Snippets are also **site-specific** — confirm you're working in the correct site.
</Accordion>

<Accordion title="Are snippets shared across sites?">
  No. Snippets are scoped to a site, so you manage and apply them per site.
</Accordion>

<Accordion title="Can I restrict a snippet to certain appointment types?">
  Yes. Use the **Appointment Types for Scribe** field to limit a snippet to specific appointment types. Leave it empty to make the snippet global (applies to all appointment types).
</Accordion>
