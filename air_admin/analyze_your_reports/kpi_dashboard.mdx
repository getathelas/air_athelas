...
![Asking Athelas AI for quick performance statistics](/images/air_admin/analyze_your_reports/kpi_dashboard/kpi_dashboard_1.gif)

### <Icon icon="chart-line" iconType="duotone" color="#F9345F" size={23} />  **Review detailed reports**

Access a full overview of KPIs in **Insights → Performance Analysis → KPI Dashboard**.

- **Definitions**: Hover over the info icon on a card to see what the metric means.
- **Filters**: Filter by **Date Range**, **Facility**, and **Provider**. Most cards, tables, and downloads respect these filters.
  - Exceptions include **Active Patients**, **Scheduled Patients**, and **Dropped Patients** (these are based on case status rather than date range).
  - Some appointment cards include their own dropdown filters so you can quickly view appointment metrics by status.
- **Trends**: Each card shows a “last period” comparison. _Last period_ is the prior interval with the same length as your date range (for example, a 7‑day range compares to the prior 7 days).
- **Export**: Use the download icon on a card (or table) to export results to CSV for follow-up.

![KPI Dashboard in Insights Performance Analysis](/images/air_admin/analyze_your_reports/kpi_dashboard/kpi_dashboard_2.webp)

**Note:** Clicking a card opens the detailed table behind that KPI. For example, **Underscheduled Patients** includes patient IDs, Plan of Care dates, and completion percentage—each patient ID links to the patient profile for quick action.

![Example detail table from a KPI card (Underscheduled Patients)](/images/air_admin/analyze_your_reports/kpi_dashboard/kpi_dashboard_4.webp)
...
Use **Active Patients Breakdown** to understand how well your active patients are scheduled—and who needs follow-up.

**To review active patient status:**

1. Go to **Insights → Performance Analysis → Active Patients Breakdown**.
2. Set your **Date Range**, **Facility**, and **Provider** filters.
3. Review the summary cards for the percentage of active patients with **0**, **1**, **2**, and **3+** scheduled visits.
4. Use the table to identify patients who are at risk of dropping off and take action (for example, schedule future visits or reach out).

**Note:** You can export the summary cards and table for outreach workflows. Exports include fields like patient, case, provider, insurance, visit counts, arrival %, last appointment, and upcoming appointments.

![Active Patients Breakdown table in Performance Analysis](/images/air_admin/analyze_your_reports/kpi_dashboard/kpi_dashboard_5.webp)
