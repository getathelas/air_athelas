...
When Athelas receives a remittance, the patient responsibility (PR) it specifies is often different from what the patient paid at the time of service. This guide explains how payments and PR are reconciled.
...
- Athelas records **Estimated Patient Responsibility** based on the amount paid — a *placeholder* meant to be replaced by finalized PR once a remittance arrives.
...
Once the payer sends a remittance, Athelas updates the total PR:

- The PR for that date of service is updated to the amount in the remittance.
- The PR type changes from **Estimated PR** to **Remittance PR**.
...
Up-front collection will never perfectly match the payer's final decision. For the difference, Athelas automatically takes the appropriate follow-up — adding a balance or issuing a credit — so your staff can focus on patients.
